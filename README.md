...
本仓库提供指令与工作方法，不包含 SketchUp 应用、操作连接器或渲染引擎。原生 `.skp` 的创建和验收需要实际 SketchUp 执行环境；环境不足时只能交付待运行的 `.rb`。专业渲染流程尚未集成。

技能已通过结构检查和 Codex 技能发现检查；此前完成三类指令情境检查，本轮补强完成结构与文件一致性检查。尚未通过真实 SketchUp 项目验证几何生成、连续改稿或原生文件保存。
